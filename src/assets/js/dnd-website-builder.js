$(function()
{
    ...
    $("#dragitemslistcontainer li").on('dragstart',function(event) {
        ...
        var insertingHTML = $(this).find('.js-html').html();
        event.originalEvent.dataTransfer.setData("Text",insertingHTML);
        if (onePage.edit.onDragStart) {
          onePage.edit.onDragStart();
        }
    });
    $("#dragitemslistcontainer li").on('dragend',function() {
        ...
        if (onePage.edit.onDragEnd) {
          onePage.edit.onDragEnd();
        }
    });
    $('#clientframe').load(function()
    {
        ...
        $(clientFrameWindow.document).find('body,html').on('drop',function(event) {
            ...
            try {
              ...
              // console.log($('#clientframe').get(0).contentWindow.document.documentElement.innerHTML);
            }
            catch(e)
            {
                console.log(e);
    ...
    var DragDropFunctions =
    {
        ...
        OrchestrateDragDrop : function($element, elementRect, mousePos)
        {
            ...
            console.log($element);
            if (!~$element[0].className.indexOf('js-block') && !$element.is('body')) {
              return;
            }
    ...
    {
        var styles = "body {overflow-x: hidden;}.block{position: relative;}.block .toolbar{position:absolute;right: 0;top: 0;z-index: 999;}";
        styles += ".reserved-drop-marker{width:100%;height:2px;background:#00a8ff;position:absolute}.reserved-drop-marker::after,.reserved-drop-marker::before{content:'';background:#00a8ff;height:7px;width:7px;position:absolute;border-radius:50%;top:-2px}.reserved-drop-marker::before{left:0}.reserved-drop-marker::after{right:0}";
